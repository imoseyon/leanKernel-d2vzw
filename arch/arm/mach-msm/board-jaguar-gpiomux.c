...
#if defined(CONFIG_VIDEO_MHL_V1) || defined(CONFIG_VIDEO_MHL_V2)
	/*MHL REV*/
	msm8960_mhl_configs[0].gpio = gpio_rev(MHL_SEL);
	msm8960_mhl_configs[1].gpio = gpio_rev(MHL_EN);
	msm8960_mhl_configs[4].gpio = gpio_rev(MHL_SDA);
	msm_gpiomux_install(msm8960_mhl_configs,
			ARRAY_SIZE(msm8960_mhl_configs));
#endif

	msm8960_mdp_vsync_configs[0].gpio = gpio_rev(MDP_VSYNC);
