...
static int enable = 1;
static int enable_user = 1;
static int reset_reason = 0xFFEEFFEE;
static char sec_build_info[100];
static unsigned int secdbg_paddr;
static unsigned int secdbg_size;
#ifdef CONFIG_SEC_SSR_DEBUG_LEVEL_CHK
static int enable_cp_debug = 1;
#endif

uint runtime_debug_val;

module_param_named(enable, enable, int, S_IRUGO | S_IWUSR | S_IWGRP);
module_param_named(enable_user, enable_user, int, S_IRUGO | S_IWUSR | S_IWGRP);
module_param_named(reset_reason, reset_reason, int, S_IRUGO | S_IWUSR | S_IWGRP);
module_param_named(runtime_debug_val, runtime_debug_val, int, S_IRUGO | S_IWUSR | S_IWGRP);
#ifdef CONFIG_SEC_SSR_DEBUG_LEVEL_CHK
module_param_named(enable_cp_debug, enable_cp_debug, int, S_IRUGO | S_IWUSR | S_IWGRP);
#endif

static int force_error(const char *val, struct kernel_param *kp);
module_param_call(force_error, force_error, NULL, NULL, S_IRUGO | S_IWUSR | S_IWGRP);

static int sec_debug_set_cpu_affinity(const char *val, struct kernel_param *kp);
module_param_call(setcpuaff, sec_debug_set_cpu_affinity, NULL, NULL, S_IRUGO | S_IWUSR | S_IWGRP);
...
void print_modem_dump_info(void)
{
	...
	if (!secdbg_modem)
		return;
	pr_info("secdbg_modem address : 0x%x", (unsigned int)secdbg_modem);
