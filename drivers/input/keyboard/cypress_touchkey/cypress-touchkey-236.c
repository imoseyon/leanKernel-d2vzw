				  ...
				  const char *buf, size_t size)
{
	...
	dev_dbg(&info->client->dev, "[TouchKey] touchkey firmware update\n");
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	dev_err(dev, "[TOUCHKEY] touch_update_read!\n");
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (ret < 0) {
		dev_err(&info->client->dev,
			"[Touchkey] fail to update menukey sensitivity data.\n");
		return ret;
	}
	msleep(150);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (ret < 0) {
		dev_err(&info->client->dev,
			"[Touchkey] fail to update backkey sensitivity data.\n");
		return ret;
	}
	msleep(150);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (ret < 0) {
		dev_err(&info->client->dev,
			"[Touchkey] fail to update raw data0.\n");
		return ret;
	}
	msleep(150);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (ret < 0) {
		dev_err(&info->client->dev,
			"[Touchkey] fail to update raw data1.\n");
		return ret;
	}
	msleep(150);
				 ...
				 size_t size)
{
	struct cypress_touchkey_info *info = dev_get_drvdata(dev);
	int count = 0;
	int on_off = kstrtoul(buf, (int)NULL, 0);

	dev_err(&info->client->dev, "[TouchKey] Test Mode : %d\n", on_off);
				 ...
				 size_t size)
{
	...
	dev_err(&info->client->dev, "called %s\n", __func__);

	ret = i2c_smbus_write_i2c_block_data(info->client,
				CYPRESS_GEN, ARRAY_SIZE(data), data);
				  ...
				  const struct i2c_device_id *id)
{
	...
	if (ret < 0) {
		dev_err(&client->dev, "gpio_touchkey_led gpio_request is failed\n");
		goto err_gpio_request;
	}
	gpio_tlmm_config(GPIO_CFG(info->pdata->gpio_led_en, 0, GPIO_CFG_OUTPUT,
