...
#define TOUCHSCREEN_SUPPORT_SYNA_SURFACE 1
...
{
	syna_pdt_scan(ts);
	pr_info("11111111111reset_all_fingers.");
	syna_set_page(ts, 0x01);
	pr_info("222222222reset_all_fingers.");
	syna_pdt_scan(ts);
	pr_info("333333333reset_all_fingers.");
	syna_set_page(ts, 0x04);
	pr_info("4444444444reset_all_fingers.");
	syna_pdt_scan(ts);
	pr_info("55555555reset_all_fingers.");
	syna_set_page(ts, 0x00);
	pr_info("66666666reset_all_fingers.");
	return 0;
}
...
#ifdef TOUCHSCREEN_SUPPORT_SYNA_SURFACE
	syna_set_surface_threshold(ts);
#endif
	pr_info("init_tsp done.");
	return;
}
...
static bool fw_updater(struct ts_data *ts, char *mode)
{
	...
	if (!strncmp("force", mode, sizeof(mode))) {
		...
		if (strncmp(FW_KERNEL_VERSION, FW_IC_VERSION,
			sizeof(FW_IC_VERSION)) > 0) {
...
#else
#ifdef TOUCHSCREEN_SUPPORT_SYNA_SURFACE
				pr_info("%d dn. remain: %d palm: %d.\n",
							id,
							++(ts->finger_cnt),
							palm);
#else
				pr_info("%d dn. remain: %d.\n",
							id, ++(ts->finger_cnt));
#endif
#endif
