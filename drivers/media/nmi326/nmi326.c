...
static irqreturn_t isdbt_irq_handler(int irq, void *dev_id)
{
	...
	struct ISDBT_OPEN_INFO_T *pdev = (struct ISDBT_OPEN_INFO_T *)dev_id;
...
{
	struct ISDBT_OPEN_INFO_T *pdev = NULL;

	NM_KMSG("<isdbt> isdbt open\n");

	/*pdev = (struct ISDBT_OPEN_INFO_T *)kmalloc(*/
	pdev = kmalloc(
				sizeof(struct ISDBT_OPEN_INFO_T),
				GFP_KERNEL);
	if (pdev == NULL) {
	...
	memset(pdev, 0x00, sizeof(struct ISDBT_OPEN_INFO_T));
...
static int isdbt_read(struct file *filp, char *buf, size_t count, \
	loff_t *f_pos)
{
	int rv = 0;
	struct ISDBT_OPEN_INFO_T *pdev;
...
static int isdbt_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_ISDBT_INTERRUPT_REGISTER:
	{
		...
		NM_KMSG(
			"<isdbt> ioctl: interrupt register, (stat : %d)\n",
			pdev->irq_status);
	...
	case IOCTL_ISDBT_INTERRUPT_UNREGISTER:
	{
		...
		NM_KMSG(
			"<isdbt> ioctl: interrupt unregister, (stat : %d)\n",
			pdev->irq_status);
	...
	case IOCTL_ISDBT_INTERRUPT_ENABLE:
	{
		...
		NM_KMSG(
			"<isdbt> ioctl: interrupt enable, (stat : %d)\n",
			pdev->irq_status);
	...
	case IOCTL_ISDBT_INTERRUPT_DISABLE:
	{
		...
		NM_KMSG(
			"<isdbt> ioctl: interrupt disable, (stat : %d)\n",
			pdev->irq_status);
	...
	case IOCTL_ISDBT_INTERRUPT_DONE:
	{
		...
		NM_KMSG(
			"<isdbt> ioctl: interrupt done, (stat : %d)\n",
			pdev->irq_status);
		if (pdev->irq_status == DTV_IRQ_INIT) {
			enable_irq(gpio_cfg.irq);
			pdev->irq_status = DTV_IRQ_SET;
	...
	case IOCTL_ISDBT_INTERRUPT_HANDLER_START:
		NM_KMSG(
			"<isdbt> IOCTL_ISDBT_INTERRUPT_START\n");
		break;
