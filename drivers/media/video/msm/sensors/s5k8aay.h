...
#define cam_info(fmt, arg...)	\
		do { printk(KERN_INFO "[s5k8aay]" fmt "\n", ##arg); } \
		while (0)

#define cam_err(fmt, arg...)	\
		do { printk(KERN_ERR "[s5k8aay] %s:%d:" fmt "\n",\
				__func__, __LINE__, ##arg); } \
		while (0)
#else
#define CAM_DEBUG(fmt, arg...)
...
#define PREVIEW_SIZE_QCIF	6    /* 176x144 */
#define PREVIEW_SIZE_MMS	7    /* 528x432 */
