...
struct akm8963_data {
	...
	u8 asa[3];
	struct mutex lock;
	struct akm8963_platform_data *akm_pdata;
	struct device *mag_sensor_device;
};
...
static int akm8963_wait_for_data_ready(struct akm8963_data *akm)
{
	...
	while (1) {
		...
		err = i2c_smbus_read_i2c_block_data(akm->i2c,
	...
	struct akm8963_data *akm)
{
	...
	/* Set measure mode */
	buffer[0] = AK8963_REG_CNTL1;
	buffer[1] = AK8963_CNTL1_POWER_DOWN;
	err = akm8963_i2c_txdata(akm->i2c, buffer, 2);
	if (err < 0) {
		dev_err(&akm->i2c->dev,
	...
	unsigned char mode)
{
	...
	switch (mode & 0x1F) { //16 bit resolution set 
...
#else
	err = akm8963_wait_for_data_ready(s_akm);
	if (err) {
	...
	struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	err = AKECS_SetMode(akm, AK8963_CNTL1_FUSE_ACCESS);
	if (err < 0)
		return err;
	...
	err = AKECS_SetMode(akm, AK8963_CNTL1_POWER_DOWN);
	if (err < 0)
		return err;
...
static irqreturn_t akm8963_irq(int irq, void *handle)
{
	...
	err = akm8963_wait_for_data_ready(akm);
	if (err) {
		pr_err("%s: wait for data ready failed\n", __func__);
		goto work_func_end;
	}
...
retry:
	...
	while (1) {
		msleep(20);
		if (i2c_smbus_read_byte_data(ak_data->i2c,
						AK8963_REG_ST1) == 1) {
			break;
		}
	}
		...
		struct device_attribute *attr, char *strbuf)
{
	...
	/* power down */
	i2c_smbus_write_byte_data(s_akm->i2c,
		AK8963_REG_CNTL1, AK8963_CNTL1_POWER_DOWN);

	/* get the value */
	i2c_smbus_read_i2c_block_data(s_akm->i2c,
					AK8963_REG_WIA, 11, buf);

	buf[11] = i2c_smbus_read_byte_data(s_akm->i2c,
					AK8963_REG_ASTC);
	buf[12] = i2c_smbus_read_byte_data(s_akm->i2c,
					AK8963_REG_I2CDIS);
		...
		struct device_attribute *attr, char *strbuf)
{
	...
	/* power down */
	err = i2c_smbus_write_byte_data(s_akm->i2c,
		AK8963_REG_CNTL1, AK8963_CNTL1_POWER_DOWN);

	buf = i2c_smbus_read_byte_data(s_akm->i2c,
					AK8963_REG_CNTL1);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if ((s_akm->asa[0] == 0) | (s_akm->asa[0] == 0xff)
		| (s_akm->asa[1] == 0) | (s_akm->asa[1] == 0xff)
		| (s_akm->asa[2] == 0) | (s_akm->asa[2] == 0xff))
		success = 0;
	else
		success = 1;
		...
		struct device_attribute *attr, char *strbuf)
{
	...
	/* start ADC conversion */
	 AKECS_Set_CNTL1(s_akm, AK8963_CNTL1_SNG_MEASURE);
	...
	err = i2c_smbus_read_i2c_block_data(s_akm->i2c,
					AK8963_REG_ST1, sizeof(buf), buf);
	if (err != sizeof(buf)) {
		pr_err("%s: read data over i2c failed\n", __func__);
		return err;
		...
		struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&s_akm->lock);
	AKECS_Set_CNTL1(s_akm, AK8963_CNTL1_SNG_MEASURE);
	ret = akm8963_wait_for_data_ready(s_akm);
	if (ret) {
		mutex_unlock(&s_akm->lock);
		goto done;
	}
	ret = i2c_smbus_read_i2c_block_data(s_akm->i2c, AK8963_REG_ST1,
						sizeof(data), data);
	mutex_unlock(&s_akm->lock);
...
int akm8963_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	s_akm->akm_pdata = pdata;
	mutex_init(&s_akm->lock);
	if (pdata) {
		...
		pr_info("%s: rst pin num: %d\n", __func__, s_akm->rstn);
		gpio_free(s_akm->rstn);
		err = gpio_request(s_akm->rstn, "gpio_akm_rstn");
		if (unlikely(err < 0)) {
			pr_err("%s: gpio %d request failed (%d)\n",
					__func__, s_akm->rstn, err);
		}
		
		err = gpio_direction_output(s_akm->rstn, 1);
		if (unlikely(err < 0)) {
	...
	akm8963_selftest(s_akm, sf);

	err = sensors_register(s_akm->mag_sensor_device,
...
static void akm8963_shutdown(struct i2c_client *client)
{
	...
	input_unregister_device(akm->input);
	mutex_destroy(&s_akm->lock);
	if (akm->akm_pdata->power_on)
		akm->akm_pdata->power_on(false);
	kfree(akm);
	dev_dbg(&client->dev, "successfully shutdown.");
}
...
static const struct dev_pm_ops akm8963_dev_pm_ops = {
