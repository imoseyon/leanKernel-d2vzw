		...
		struct device_attribute *attr, char *strbuf)
{
	...
	return snprintf(strbuf, PAGE_SIZE,
		...
		struct device_attribute *attr, char *strbuf)
{
	...
	return snprintf(strbuf, PAGE_SIZE, "%s\n", (!buf ? "OK" : "NG"));
}
...
static int akm8975_suspend(struct i2c_client *dev, pm_message_t mesg)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct akm8975_data *akm = i2c_get_clientdata(client);
	akm8975_ecs_set_mode_power_down(akm);
	if (akm->pdata->power_on)
		akm->pdata->power_on(0);
	 return 0;
}

static int akm8975_resume(struct i2c_client *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct akm8975_data *akm = i2c_get_clientdata(client);
	if (akm->pdata->power_on)
...
static const struct dev_pm_ops ak_dev_pm_ops = {
