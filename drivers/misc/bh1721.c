	...
	struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "%s\n",
		       ...
		       (bh1721fvc->measure_mode ==
			L_MEASURE) ? "low" : "invalid");
}
...
static struct device_attribute dev_attr_light_sensor_lux =
	__ATTR(lux, S_IRUSR | S_IRGRP, bh1721fvc_light_sensor_lux_show, NULL);
static struct device_attribute dev_attr_light_sensor_raw_data =
	__ATTR(raw_data, S_IRUSR | S_IRGRP,
		bh1721fvc_light_sensor_lux_show, NULL);

static ssize_t bh1721fvc_light_vendor_show(struct device *dev,
					   struct device_attribute *attr,
					   char *buf)
{
	return sprintf(buf, "%s\n", "ROHM");
...
static struct device_attribute dev_attr_light_sensor_vendor =
	__ATTR(vendor, S_IRUSR | S_IRGRP, bh1721fvc_light_vendor_show, NULL);

static ssize_t bh1721fvc_light_name_show(struct device *dev,
					   struct device_attribute *attr,
					   char *buf)
{
	return sprintf(buf, "%s\n", "BH1721");
