				      ...
				      const char *buf, size_t size)
{
	...
	if (new_value && !(cm36651->power_state & PROXIMITY_ENABLED)) {
		u8 val = 1;
		int i;
		cm36651->pdata->cm36651_led_on(1);
		msleep(20);
#ifdef CM36651_CANCELATION
		...
		cm36651_i2c_write_byte(cm36651, CM36651_PS, PS_CONF1,
				       0x01);
		cm36651->pdata->cm36651_led_on(0);
	}
	mutex_unlock(&cm36651->power_lock);
	return size;
}
			     ...
			     const struct i2c_device_id *id)
{
	...
	if (pdata->cm36651_power_on)
		pdata->cm36651_power_on(1);
	if  (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		pr_err("%s: i2c functionality check failed!\n", __func__);
		if (pdata->cm36651_power_on)
			pdata->cm36651_power_on(0);
		return ret;
	}
...
static int cm36651_suspend(struct device *dev)
{
	...
	if (!(cm36651->power_state & PROXIMITY_ENABLED)) {
		gpio_free(cm36651->pdata->irq);
		if (cm36651->pdata->cm36651_power_on)
			cm36651->pdata->cm36651_power_on(0);
	}
...
static int cm36651_resume(struct device *dev)
{
	...
	int ret = 0;
	if (!(cm36651->power_state & PROXIMITY_ENABLED)) {
		ret = gpio_request(cm36651->pdata->irq, "gpio_proximity_out");
		if (ret) {
			pr_err("%s gpio request %d err\n", __func__,
				cm36651->pdata->irq);
		}
		if (cm36651->pdata->cm36651_power_on)
			cm36651->pdata->cm36651_power_on(1);
	}
	if (cm36651->power_state & LIGHT_ENABLED)
		cm36651_light_enable(cm36651);
