...
static struct prod_rev_map_t prod_rev_map[] = {
	...
	/* prod_ver = 5 */
	{MPL_PROD_KEY(6,  19), MPU_SILICON_REV_B1, 131, 16384},	/* (B5/E1)   */
 ...
 *          The product revision number is in read from OTP bank 0, 
 *		ADDR6[7:2].
 ...
 *
 *          Putting the device into sleep mode depending upon the changing 
 *          needs of the associated applications is a recommended method 
 *          for reducing power consuption.  It is a safe opearation in that 
 *          sleep/wake up of gyros while running will not result in any 
 *		interruption of data.
 *
 *          Although it is entirely allowed to put the device into full sleep
 *          while running the DMP, it is not recomended because it will 
 *          disrupt the ongoing calculations carried on inside the DMP 
 *          and consequently the sensor fusion algorithm. Furthermore, 
 *          while in sleep mode read & write operation from the app 
 *          processor on both registers  and memory are disabled and 
 *          can only regained by restoring the  MPU in normal power mode.
