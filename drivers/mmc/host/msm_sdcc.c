...
static int msmsdcc_msm_bus_register(struct msmsdcc_host *host)
{
	...
	if (host->plat->msm_bus_scale_data) {
		host->bus_client_handle = msm_bus_scale_register_client(
				host->plat->msm_bus_scale_data);
		if (!host->bus_client_handle) {
			pr_err("%s: msm_bus_scale_register_client() failed"
			    " with err=%d\n", mmc_hostname(host->mmc),
			    host->bus_client_handle);
			rc = host->bus_client_handle;
		}
	}
...
{
	if (host->bus_client_handle)
		msm_bus_scale_unregister_client(host->bus_client_handle);
}
