...
------------------------------------------------------------------------------*/
byte sii_regio_cub_read(word regAddr, byte channel)
{
	return i2c_read_byte(SA_TX_CBUS_Primary + l_cbusPortOffsets[channel],
								regAddr);
}
...
------------------------------------------------------------------------------*/
void sii_regio_cbus_write(word regAddr, byte channel, byte value)
{
	i2c_write_byte(SA_TX_CBUS_Primary + l_cbusPortOffsets[channel], regAddr,
									value);
}
