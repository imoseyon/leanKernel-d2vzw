...
enum {
	...
	/*The key code in the RCP sub-command is not recognized*/
	MHD_MSC_MSG_INEFFECTIVE_KEY_CODE = 0x01,
...
enum {
	...
	MHD_WRITE_STAT = 0x60 | 0x80,/*Write one status register strip topbit*/
...
struct cbusChannelState_t {
	...
	byte state;          /*State of command execution for this channel*/
				/* IDLE, WAIT_RESPONSE*/
/*Timer for message response (must be able to indicate at least 3000ms).*/
	byte timeout;
...
bool si_cbus_write_command(byte channel, struct cbus_req_t *pReq);
