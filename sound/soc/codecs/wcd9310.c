...
#define MBHC_USE_HPHL_TRIGGER 0x01
#define MBHC_USE_MB_TRIGGER 0x02
...
#define TABLA_FAKE_REMOVAL_MIN_PERIOD_MS 20
...
struct tabla_priv {
	...
	struct snd_soc_jack *volumeup_jack;
	struct snd_soc_jack *volumedown_jack;
...
static void tx_hpf_corner_freq_callback(struct work_struct *work)
{
	...
	hpf_cut_of_freq = tabla->tx_hpf_cut_of_freq[hpf_work->decimator - 1];

	tx_mux_ctl_reg = TABLA_A_CDC_TX1_MUX_CTL +
		(hpf_work->decimator - 1) * 8;
	pr_debug("%s(): tabla %p decimator %u hpf_cut_of_freq 0x%x\n",
		__func__, hpf_work->tabla, hpf_work->decimator,
		(unsigned int)hpf_cut_of_freq);
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		tabla->tx_hpf_cut_of_freq[decimator - 1] = dec_hpf_cut_of_freq;


		if ((dec_hpf_cut_of_freq == CF_MIN_3DB_4HZ) ||
			(dec_hpf_cut_of_freq == CF_MIN_3DB_75HZ)) {
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		if (tabla->tx_hpf_cut_of_freq[decimator - 1] ==
			CF_MIN_3DB_150HZ)
			break;
		schedule_delayed_work(
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		snd_soc_update_bits(codec, tx_mux_ctl_reg, 0x30,
				tabla->tx_hpf_cut_of_freq[decimator - 1] << 4);
		break;
	}
out:
	kfree(widget_name);
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		snd_soc_write(codec, rx_digital_gain_reg[w->shift],
...
static const struct snd_soc_dapm_widget tabla_dapm_widgets[] = {
	...
	SND_SOC_DAPM_AIF_OUT("SLIM TX3", "AIF1 Capture", NULL, SND_SOC_NOPM,
			0, 0),
...
static int tabla_volatile(struct snd_soc_codec *ssc, unsigned int reg)
{
	 ...
	 */
	if ((reg == TABLA_A_CDC_RX1_VOL_CTL_B2_CTL) ||
		(reg == TABLA_A_CDC_RX2_VOL_CTL_B2_CTL) ||
		(reg == TABLA_A_CDC_RX3_VOL_CTL_B2_CTL) ||
		(reg == TABLA_A_CDC_RX4_VOL_CTL_B2_CTL) ||
		(reg == TABLA_A_CDC_RX5_VOL_CTL_B2_CTL) ||
		(reg == TABLA_A_CDC_RX6_VOL_CTL_B2_CTL) ||
		(reg == TABLA_A_CDC_RX7_VOL_CTL_B2_CTL))
		return 1;
					...
					bool padac_off)
{
	...
	if (insertion) {
		...
		if (padac_off == true) {
			...
			snd_soc_update_bits(codec, TABLA_A_RX_HPH_CNP_EN,
				 0x30, 0x00);
		...
		if (trigger == MBHC_USE_HPHL_TRIGGER) {
			...
			snd_soc_update_bits(codec,
				tabla->mbhc_bias_regs.mbhc_reg, 0x60,
				plug_det->mic_current << 5);
			snd_soc_update_bits(codec,
				tabla->mbhc_bias_regs.mbhc_reg,	0x80, 0x80);
			usleep_range(plug_det->t_mic_pid, plug_det->t_mic_pid);
			snd_soc_update_bits(codec,
				tabla->mbhc_bias_regs.ctl_reg, 0x01, 0x00);
			snd_soc_update_bits(codec,
...
static void btn_lpress_fn(struct work_struct *work)
{
	...
	tabla = container_of(delayed_work, struct tabla_priv, btn_dwork);
	...
	if (tabla) {
		if (tabla->button_jack) {
			...
			btn = tabla->buttons_pressed & TABLA_JACK_BUTTON_MASK;
			if (btn == SND_JACK_BTN_0) {
				pr_debug("%s: (Long) SND_JACK_BTN_0\n",
					__func__);
				snd_soc_jack_report(tabla->button_jack,
					 SND_JACK_BTN_0, SND_JACK_BTN_0);
			} else if (btn == SND_JACK_BTN_1) {
				pr_debug("%s: (Long) button Vol Up %d press\n",
					__func__, btn);
				snd_soc_jack_report(tabla->volumeup_jack,
					 SND_JACK_BTN_1, SND_JACK_BTN_1);
			} else if (btn == SND_JACK_BTN_2) {
				pr_debug("%s: (Long) Vol Down %d press\n",
					__func__, btn);
				snd_soc_jack_report(tabla->volumedown_jack,
					 SND_JACK_BTN_2, SND_JACK_BTN_2);
			}
			/* For Samsung Factorytest */
...
void tabla_mbhc_init(struct snd_soc_codec *codec)
{
	...
	if (machine_is_jaguar()) {
		...
		snd_soc_update_bits(codec, TABLA_A_MBHC_SCALING_MUX_2,
			0xF0, 0xC0);
	} else {
...
static irqreturn_t tabla_dce_handler(int irq, void *data)
{
	...
	u8 mbhc_status =
		snd_soc_read(codec, TABLA_A_CDC_MBHC_B1_STATUS) & 0x3E;
	...
	dce_hw = bias_value_dce = tabla_codec_read_dce_result(codec);
	sta_hw = tabla_codec_read_sta_result(codec);

	if (priv->resume_jiffy != 0
			&& !time_after(jiffies, priv->resume_jiffy + (3 * HZ)))
		from_suspend = true;

	/* If suspend did not happen earlier and button is already released */
	if (!from_suspend && (mbhc_status != 0x0C)) {
		pr_debug("%s: not from suspend and button already released\n",
			__func__);
		new_btn = btn = -1;
		goto decide_button;
	}
	...
	pr_info("%s: meas %d - DCE %d,%d, button %d\n", __func__,
	...
	/* button press was not mapped to any button.
	 * this is possible for very short button press.
	 */
	new_btn = btn;

	if (from_suspend) {
		pr_debug("%s: Button press after resuming from suspend\n",
				__func__);
		if (mbhc_status != 0x0C) {
			int new_btn_dce, new_btn_sta;
			pr_debug("%s: Button already released\n", __func__);
			new_btn_dce = tabla_determine_button(priv,
			tabla_codec_sta_dce_v(codec,
				 1, dce_hw));

			new_btn_sta = tabla_determine_button(priv,
			tabla_codec_sta_dce_v(codec,
				 0, sta_hw));

			if (new_btn_dce > -1
				&& new_btn_sta > -1
				&& new_btn_dce == new_btn_sta) {
				pr_debug("%s: button %d detected\n", __func__,
					new_btn_sta);
				new_btn = new_btn_dce;
			}
		}
...
decide_button:
	if (new_btn >= 0) {
		mask = tabla_get_button_mask(new_btn);
		priv->buttons_pressed |= mask;
		msleep(100);
		tabla_lock_sleep(core);
		if (schedule_delayed_work(&priv->btn_dwork,
...
enum { MBHC_STATE_MIC_IN, MBHC_STATE_MIC_REMOVED, MBHC_STATE_MIC_UNSTABLE };

static int tabla_mbhc_settle_wait(struct tabla_priv *priv, int timeout)
{
	int i, r = -ETIMEDOUT;
	int states[3];
	short bias_value;
	struct snd_soc_codec *codec = priv->codec;
	unsigned long to = jiffies + msecs_to_jiffies(timeout);

	pr_debug("%s: enter\n", __func__);
	while (!time_after(jiffies, to)) {
		for (i = 0; i < sizeof(states)/sizeof(states[0]); i++) {
			bias_value = tabla_codec_sta_dce(codec, 1);
			if (bias_value < (short)priv->mbhc_data.v_ins_h &&
			    bias_value > (short)priv->mbhc_data.v_b1_h) {
				/* regular mic voltage */
				pr_debug("%s: i %d, mic is in\n", __func__, i);
				states[i] = MBHC_STATE_MIC_IN;
			} else if (bias_value >
				   (short)priv->mbhc_data.v_ins_h) {
				pr_debug("%s: i %d, mic is removed\n",
					 __func__, i);
				states[i] = MBHC_STATE_MIC_REMOVED;
			} else if (bias_value <
				   (short)priv->mbhc_data.v_b1_h) {
				states[i] = MBHC_STATE_MIC_UNSTABLE;
				break;
			}
		}
		if ((states[0] != MBHC_STATE_MIC_UNSTABLE) &&
		    ((states[0] == states[1]) && (states[1] == states[2]))) {
			r = states[0];
			break;
		}
	}
	pr_debug("%s leave r = %d\n", __func__, r);
...
static irqreturn_t tabla_release_handler(int irq, void *data)
{
	...
	mutex_unlock(&priv->mbhc_mutex);
	tabla_disable_irq(codec->control_data, TABLA_IRQ_MBHC_RELEASE);
	btn = priv->buttons_pressed & TABLA_JACK_BUTTON_MASK;

	if (btn) {
		ret = cancel_delayed_work(&priv->btn_dwork);
		if (ret == 0) {
			pr_debug("%s: Reporting long button release event\n",
				 __func__);

			if (priv->button_jack && btn == SND_JACK_BTN_0) {
				pr_info("%s: (Long Release) "
						"SND_JACK_BTN_0\n", __func__);
				snd_soc_jack_report(priv->button_jack,
					 0, SND_JACK_BTN_0);
			} else if (priv->volumeup_jack &&
					 btn == SND_JACK_BTN_1) {
				pr_info("%s: (Long Release) "
						"Vol Up %d\n",
						 __func__, btn);
				snd_soc_jack_report(priv->volumeup_jack,
					 0, SND_JACK_BTN_1);
			} else if (priv->volumedown_jack &&
					 btn == SND_JACK_BTN_2) {
				pr_info("%s: (Long Release) "
						"Vol Down %d\n",
						 __func__, btn);
				snd_soc_jack_report(priv->volumedown_jack,
					 0, SND_JACK_BTN_2);
			}
			/* For Samsung Factorytest */
			...
			if (tabla_is_fake_press(priv)) {
				pr_info("%s: Fake button press interrupt\n",
					 __func__);
				msleep(500);
				tabla_mbhc_settle_wait(priv, 5000);
			} else {
				if (priv->button_jack &&
						 btn == SND_JACK_BTN_0) {
					pr_info("%s: Short button 0 "
						"press and release\n",
						 __func__);
...
bool is_fake_insertion(struct snd_soc_codec *codec,
	s32 mic_volt)
{
	struct tabla_priv *tabla = snd_soc_codec_get_drvdata(codec);
	struct tabla_mbhc_plug_type_cfg *plug_type =
...
void tabla_codec_detect_plug_type(struct snd_soc_codec *codec)
{
	struct tabla_priv *tabla = snd_soc_codec_get_drvdata(codec);
	const struct tabla_mbhc_plug_detect_cfg *plug_det =
	    TABLA_MBHC_CAL_PLUG_DET_PTR(tabla->calibration);
	...
	u8 plug_type[NUM_DCE_PLUG_DETECT];
	...
	if (is_fake_insertion(codec, mic_mv[0])) {
		pr_debug("detect attempt 1, detected Fake\n");
		tabla_codec_shutdown_hs_polling(codec);
		tabla_codec_enable_hs_detect(codec, 1, MBHC_USE_MB_TRIGGER,
					     false);
		return;
	}
	pr_debug("Run 1, mic_mv = %d\n", mic_mv[0]);
	 ...
	 */
	snd_soc_update_bits(codec, TABLA_A_CDC_MBHC_B1_CTL, 0x02, 0x00);
	for (i = 1; i < NUM_DCE_PLUG_DETECT; i++) {
		mb_v[i] = tabla_codec_sta_dce(codec, 1);
		mic_mv[i] = tabla_codec_sta_dce_v(codec, 1 , mb_v[i]);
		pr_debug("Run %d, mic_mv = %d\n", i + 1, mic_mv[i]);
		if (is_fake_insertion(codec, mic_mv[i])) {
			pr_debug("detect attemp %d, detected Fake\n", i + 1);
			snd_soc_update_bits(codec, TABLA_A_CDC_MBHC_B1_CTL,
				0x02, 0x02);
			tabla_codec_shutdown_hs_polling(codec);
	...
	for (i = 0; i < NUM_DCE_PLUG_DETECT; i++) {
		if (mic_mv[i] < plug_type_ptr->v_no_mic) {
			...
			pr_debug("detect attempt %d, detected Headphone\n", i);
		} else {
			plug_type[i] = PLUG_TYPE_HEADSET;
			pr_debug("detect attempt %d, detected Headset\n", i);
		}

		if (i > 0 &&
			(plug_type[i - 1] != plug_type[i])) {
	...
	if (plug_type[0] == PLUG_TYPE_HEADPHONE) {
		...
		if (tabla->headset_jack) {
			pr_info("%s: Reporting insertion %d\n", __func__,
				SND_JACK_HEADPHONE);
			switch_set_state(&switch_jack_detection,
						SEC_HEADSET_3POLE);
			tabla_snd_soc_jack_report(tabla, tabla->headset_jack,
						  tabla->hph_status,
						  TABLA_JACK_MASK);
...
static irqreturn_t tabla_hs_insert_irq(int irq, void *data)
{
	...
	snd_soc_update_bits(codec, TABLA_A_MBHC_HPH, 0x13, 0x00);
	snd_soc_update_bits(codec, priv->mbhc_bias_regs.ctl_reg,
		0x01, 0x00);
...
static irqreturn_t tabla_hs_remove_irq(int irq, void *data)
{
	...
	int fake_removal = 0, fake_removal_settle;
	...
	if (fake_removal) {
		pr_debug("%s: waiting for fake removal settling down\n",
			 __func__);
		msleep(500);
		fake_removal_settle = tabla_mbhc_settle_wait(priv, 5000);
		if (fake_removal_settle == MBHC_STATE_MIC_IN)
			fake_removal = 1;
		else
			fake_removal = 0;
	}
...
{
	int value, ret;
	struct tabla_priv *tabla = dev_get_drvdata(dev);

	if (tabla->buttons_pressed & SND_JACK_BTN_0) {
		value = 1;
		ret = sprintf(buf, "%d\n", value);
		return ret;
	} else {
...

{
	...
	if (tabla->hph_status & SND_JACK_HEADSET) {
		value = 1;
		ret = sprintf(buf, "%d\n", value);
		return ret;
	} else {
		    ...
		    int read_fw_bin, u32 mclk_rate)
{
	...
	tabla->volumeup_jack = volumeup_jack;
	tabla->volumedown_jack = volumedown_jack;
	...
	INIT_DELAYED_WORK(&tabla->btn_dwork, btn_lpress_fn);
	...
	if (!read_fw_bin) {
		...
		tabla->mclk_cb(codec, 0);
		tabla_codec_calibrate_hs_polling(codec);
		rc =  tabla_codec_enable_hs_detect(codec,
				 1, MBHC_USE_MB_TRIGGER, false);
	} else {
		schedule_delayed_work(&tabla->mbhc_firmware_dwork,
				usecs_to_jiffies(MBHC_FW_READ_TIMEOUT));
	}

	rc = switch_dev_register(&switch_jack_detection);
	if (rc < 0) {
	...
	rc = device_create_file(earjack, &dev_attr_key_state);
	if (rc)
		pr_err("Failed to create device file in sysfs entries(%s)!\n",
				dev_attr_key_state.attr.name);

	rc = device_create_file(earjack, &dev_attr_state);
	if (rc)
		pr_err("Failed to create device file in sysfs entries(%s)!\n",
				dev_attr_state.attr.name);

	rc = device_create_file(earjack, &dev_attr_select_jack);
	if (rc)
		pr_err("Failed to create device file in sysfs entries(%s)!\n",
				dev_attr_select_jack.attr.name);

	dev_set_drvdata(earjack, tabla);
